...
class TruckController extends Controller
{
    ...
    public function TruckCarTypes(){
        $TruckCarTypes=SpecialCar::where('type',2)->get();
        return response()->json(['TruckCarTypes' => $TruckCarTypes]);
    }
    ...
    function haversineGreatCircleDistance(Request $request){
        ...
        $spcialistscount=$request->spcialistscount;
        $car_type_id=$request->car_type_id;
        $service_count=$request->quantity;
        $kilo=$final/1000;




        $te=SpecialCar::where('id',$car_type_id)->pluck('price')->first();

        if ($PaymentMethod == 'byrequest') {
        $costpramter= CarSetting::where('id',5)->pluck('byrequest')->first();
        $finalcost=($costpramter*$service_count*$kilo)+$te+(7*$spcialistscount)+(4*$workerscount);
            ...
            $costpramter= CarSetting::where('id',5)->pluck('byhour')->first();
            $fa=($costpramter*$service_count*$kilo)+$te+(7*$spcialistscount)+(4*$workerscount);
            ...
            $costpramter= CarSetting::where('id',5)->pluck('price')->first();
            $fa=($costpramter*$service_count*$kilo)+$te+(7*$spcialistscount)+(4*$workerscount);
            return response()->json(['distance' => $final,
            'Cost' => $fa,
            'est' => $est
    ...
    public function ConfirmRide(Request $request){
        ...
        if ($trip->save()) {
            ...
            $NewItem->number_workers = $request->number_workers;
            $NewItem->number_technicians = $request->number_technicians;
            $NewItem->cars_count = $request->cars_count;
